    ...
    try {
      if (data.operation === "embed") {
        if (!data.watermarkText) {
          const defaultText = `Devanshi`;
          const response = await watermarkAPI.addInvisibleWatermark(file, {
            text: defaultText,
          });
          setResult(`Invisible watermark ('${defaultText}') embedded successfully!`);
          setResultImage(response.watermarked_image);
          setMetadata(response.metadata || {});
        }
        else{// Use standard settings for simplicity
        const response = await watermarkAPI.addTextWatermark(file, {
          text: data.watermarkText as string,
          position: "bottom-right",
          opacity: 1, // Medium opacity
          font_size: 30,
        });
        ...
        setResult(`Extracted watermark: ${response.extracted_text}`);
        setMetadata(response.metadata || {});
      }
